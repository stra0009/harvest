...
def main_logic
  ...
  tasks.each do |task|
    embed_hash = {
      ...
      'color' => 65280, # Green
      'fields' => [
        { 'name' => "優先度", 'value' => (task['priority'] || "普通").to_s, 'inline' => true },
        { 'name' => "元メッセージ", 'value' => (task['original_context'] || "N/A").to_s[0..1000] } # Truncate to avoid limit
      ],
      'footer' => { 'text' => "Generated by Gemini • Your Senpai Ace" }
    }
    
    Discordrb::API::Channel.create_message("Bot #{DISCORD_TOKEN}", TARGET_CHANNEL_ID, nil, false, embed_hash)
